...
The following [Python file](Docker) gets transformed with `python CICD.py > DockerCICD.yml` to our wanted Cloudformation YAML template.
...
## Deploying the Cloudoformation template
...
Now, you can develop your image on your local machine. Ready? Use `zip -r docker.zip .`, get the complete bucket name of your `dockerstaticwebsite`  and upload you zip-file to S3:

````bash
aws s3 ls
aws s3 cp foo.zip s3://dockerstaticwebsite+str(RANDOM_HASH) 
